...
function safeSetWindowProperty(property, value) {
    try {
        ...
        if (descriptor && !descriptor.configurable) {
            // Log the issue but don't try to set the property
            console.log(`Property ${property} is read-only, applying fix`);
            
            // Create a safe alternative property with a different name
            const safeProp = `_${property}`;
            if (!window[safeProp]) {
